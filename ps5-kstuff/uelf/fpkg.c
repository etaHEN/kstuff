...
// int try_handle_fpkg_mailbox(uint64_t* regs, uint64_t lr)
// {
//     if (regs[RIP] == (uint64_t) sceSblServiceMailbox_lr_verifySuperBlock - 5)
//     {
//         uint64_t req[8];
//         copy_from_kernel(req, regs[RDX], 64);
//         uint64_t p_eekpfs = 0;
...
//                 if(key2 >= 0)
//                 {
//                     regs[RIP] = (uint64_t) sceSblServiceMailbox_lr_verifySuperBlock; // change for 5.5
//                     regs[RAX] = 0;
//                     // regs[RSP] += 8; // change for 5.5
//                     uint32_t fake_resp[4] = {0, 0, IDX_TO_HANDLE(key1), IDX_TO_HANDLE(key2)};
//                     copy_to_kernel(regs[RDX], fake_resp, sizeof(fake_resp));
//                     // CRASH();
//                     // current_stop_addr = (uint64_t) sceSblServiceMailbox_lr_verifySuperBlock;
//                 }
//             }
//         }
//     }
//     else if(regs[RIP] == (uint64_t)sceSblServiceMailbox_lr_sceSblPfsClearKey_1 - 5 
//             || regs[RIP]  == (uint64_t)sceSblServiceMailbox_lr_sceSblPfsClearKey_2 - 5) 
//     {
//         uint32_t handle = kpeek64(regs[RDX]+8);
//         int key = HANDLE_TO_IDX(handle);
//         if(key >= 0 && unregister_fake_key(key))
...
//             copy_to_kernel(regs[RDX], (const uint64_t[16]){}, 16);
//             // regs[RIP] = lr;
//             regs[RIP] += 5; //(uint64_t) sceSblServiceMailbox_lr_sceSblPfsClearKey_2; // change for 5.50
//             regs[RAX] = 0;
//             // regs[RSP] += 8; // change for 5.50
//         }
//     }
//     /*else
//     {
...
// // static const uint64_t dbgregs_for_nmount[6] = {
// //     (uint64_t)(uint64_t)sceSblServiceMailbox, 0, 0, 0,
// //     0, 0x401
// // };

// static const uint64_t dbgregs_for_unmount[6] = {
//     (uint64_t)(uint64_t)sceSblServiceMailbox_lr_sceSblPfsClearKey_1 - 5, (uint64_t) sceSblServiceMailbox_lr_sceSblPfsClearKey_2 - 5, 0, 0,
//     0, 0x405
// };

// static const uint64_t dbgregs_for_nmount[6] = {
//     (uint64_t)(uint64_t)sceSblServiceMailbox_lr_verifySuperBlock - 5, 0, 0, 0,
//     0, 0x401
// };

// void handle_fpkg_numount_syscall(uint64_t* regs)
// {
//     start_syscall_with_dbgregs(regs, dbgregs_for_unmount);
// }
